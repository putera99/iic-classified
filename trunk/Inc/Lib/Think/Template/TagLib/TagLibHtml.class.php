...
{//类定义开始
    ...
    public function _editor($attr)
    {
        ...
        switch(strtoupper($type)) {
			case 'CKEDITOR':
                $parseStr   =	'<!-- 编辑器调用开始 --><script type="text/javascript" src="__ROOT__/Public/Js/CKeditor/ckeditor.js"></script><textarea id="'.$id.'" name="'.$name.'">'.$content.'</textarea><script type="text/javascript"> CKEDITOR.replace("'.$id.'", {width:"'.$width.'", height:"'.$height.'"});</script> <!-- 编辑器调用结束 -->';
                break;
            case 'FCKEDITOR':
...
$parseStr .='<li style="width:25px;"><input type="checkbox" id="check" onclick="CheckAll(\''.$id.'\')"></li>'; 
} 
foreach($fields as $field) {//显示指定的字段 
$property = explode('|',$field[0]); 
$showname = explode('|',$field[1]); 
if(isset($showname[1])) { 
$parseStr .= '<li style="width:'.$showname[1].';">'; 
}else { 
$parseStr .= '<li style="width:auto;">'; 
} 
$showname[2] = isset($showname[2])?$showname[2]:$showname[0]; 
$parseStr .= $showname[0].'</li>'; 
} 
if(!empty($action)) {//如果指定显示操作功能列 
$parseStr .= '<li style="width:10%;">操作</li>'; 
} 

$parseStr .= "\n\t\t</ul>\n\t</dt>"; 
$parseStr .= '<volist name="'.$datasource.'" id="'.$name.'" >'."\n\t".'<dd onclick="change(event)">'."\n\t\t<ul>"; //支持鼠标移动单元行颜色变化具体方法在js中定义 

if(!empty($checkbox)) {//如果需要显示checkbox 则在每行开头显示checkbox 
$parseStr .= '<li style="width:25px;"><input type="checkbox" name="key" value="{$'.$name.'.'.$pk.'}"></li>'; 
} 
...
$parseStr .= '<li style="width:auto;"><a href="javascript:edit({$'.$name.'.'.$pk.'})">编辑</a> <a onfocus="javascript:getTableRowIndex(this)" href="javascript:del({$'.$name.'.'.$pk.'})">删除</a></li>'; 
} 

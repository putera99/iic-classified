 ...
 * 返回URL路径
 *
 * @param string $url - 模块信息
 * @param array $params - 参数
 * @param bool $redirect - 是否跳转
 * @param bool $suffix - 是否生成伪静态
 * @param string $re - REWRITE前缀
 * @return string
 */
function U($url,$params=array(),$redirect=false,$suffix=true) {
	$re=empty($params['re'])?0:$params['re'];
	unset($params['re']);
    if(0===strpos($url,'/'))
        $url   =  substr($url,1);
    ...
    if(C('URL_DISPATCH_ON') && C('URL_MODEL')>0) {
        ...
        //$c_str='-'.strlen(C('URL_PATHINFO_DEPR'));
        $str = substr($str,0,-1);
        ...
        if(!empty($route)) {
            ...
            $url    =   str_replace(APP_NAME,$app,$re).'/'.$group.$module.$depr.$action.$str;
        }
